    ...
    <>
      <section className="indicator-page">
        ...
        <div className="indicator-container">
          <div className="top-bar">
            ...
            <div className="search-box">Search indicator...</div>
          </div>

          <div className="indicator-grid">
            {indicators.map(([name, desc, tag], index) => {
              const isLast = index === indicators.length - 1;

              return (
                <article className={`indicator-card ${isLast ? "new-card" : ""}`} key={name}>
                  {isLast && <span className="new-flash">NEW</span>}
      ...
      <style>{`
        ...
        .indicator-bg{
          ...
          inset:0;
          background:
            radial-gradient(circle at 78% 10%, rgba(0,255,136,.22), transparent 32%),
            radial-gradient(circle at 15% 70%, rgba(0,255,136,.12), transparent 35%),
            linear-gradient(180deg,#020403,#06140d 45%,#020403);
          pointer-events:none;
        }
        ...
        .hero-orb{
          ...
          place-items:center;
          background:
            radial-gradient(circle,#123e29 0%,#07130e 55%,#00ff88 57%,#062014 60%,transparent 70%);
          box-shadow:0 0 70px rgba(0,255,136,.4);
          animation:pulseOrb 2.6s infinite ease-in-out;
        }
        ...
        .orb-inner{
          ...
          display:grid;
          place-items:center;
          border:2px solid rgba(0,255,136,.55);
          background:linear-gradient(145deg,#111,#031109);
          color:#00ff88;
          font-size:56px;
          font-weight:950;
          letter-spacing:-.08em;
        ...
        .indicator-grid{
          ...
          box-shadow:0 0 50px rgba(0,255,136,.09);
        }
        ...
        .indicator-card{
          ...
          background:linear-gradient(145deg,rgba(0,255,136,.08),rgba(0,0,0,.18));
          transition:.28s ease;
        }

        .indicator-card:hover{
          transform:translateY(-5px);
          background:linear-gradient(145deg,rgba(0,255,136,.15),rgba(0,0,0,.22));
          box-shadow:inset 0 0 30px rgba(0,255,136,.08);
        }
        ...
        .new-card{
          grid-column:span 2;
          background:linear-gradient(145deg,rgba(255,190,0,.16),rgba(0,0,0,.35));
          border:1px solid rgba(255,196,0,.8)!important;
          box-shadow:0 0 35px rgba(255,196,0,.28), inset 0 0 28px rgba(255,196,0,.08);
        }
        ...
        .new-flash{
          ...
          text-shadow:0 0 12px #ffd400,0 0 24px #ff8a00;
          animation:newBlink .9s infinite alternate;
        }

        .footer-box{
          margin-top:32px;
          background:linear-gradient(135deg,rgba(0,255,136,.12),rgba(0,0,0,.4));
        ...
        .explore-btn{
          ...
          box-shadow:0 0 25px rgba(0,255,136,.28);
        }
        ...
        @keyframes newBlink{
          from{opacity:.65;transform:rotate(-8deg) scale(.96)}
        ...
        @media(max-width:950px){
          .indicator-page{padding-top:80px}
          .indicator-hero{grid-template-columns:1fr}
          .hero-orb{display:none}
          .stats{grid-template-columns:repeat(2,1fr)}
          .top-bar{align-items:flex-start;flex-direction:column}
          .search-box{width:100%}
          .indicator-grid{grid-template-columns:repeat(2,1fr)}
          .new-card{grid-column:span 2}
        }

        @media(max-width:580px){
          .indicator-page{padding-left:14px;padding-right:14px}
          .indicator-grid{grid-template-columns:1fr}
          .new-card{grid-column:span 1}
          .stats{grid-template-columns:1fr}
          .indicator-card{min-height:auto}
          .indicator-hero h1{font-size:42px}
        }
      `}</style>
    </>
  );
